...
class GLMPrefixForSequenceClassification(GLMPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
    ...
    def get_prompt(self, batch_size):
        ...
        past_key_values = past_key_values.permute([2, 3, 0, 1, 4])
    ...
    def forward(
        ...
        # 模仿 modeling_glm 中的 GLMForSequenceClassification
...
class GLMPromptForSequenceClassification(GLMPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.glm = GLMModel(config)
    ...
    def forward(
        ...
        prompt = self.get_prompt(batch_size=batch_size)
