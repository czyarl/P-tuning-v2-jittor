...
class PositionalEmbedding(torch.nn.Module):
    ...
    # xhb
    def execute(self, pos_seq, bsz=None):
...
class SelfAttention(torch.nn.Module):
    ...
    # xhb
    def execute(self, hidden_states, ltor_mask, mem=None, past_key_values=None):
        # hidden_states: [b, s, h]
        # ltor_mask: [b,1,s,s]
        ...
        if past_key_values is not None:
            # past_key_values: [2, b, n_heads, pre_seq_len, n_embed]
    ...
    def forward(self, hidden_states, ltor_mask, mem=None, past_key_values=None):
        # hidden_states: [b, s, h]
        # ltor_mask: [b,1,s,s]
...
# xhb
    ...
    def execute(self, hidden_states, ltor_mask, mem=None, past_key_values=None):
        ...
        # Self attention.
        attention_output = self.attention(layernorm_output, ltor_mask, mem, past_key_values)
        # Residual connection.
...
# xhb
    def execute(self, hidden_states, position_ids, attention_mask, memory_states=None, past_key_values=None, prompt=None):

        batch_size, query_length = hidden_states.size()[:2]
        # memory_length = memory_states[0].size(1) if memory_states is not None else 0
        # ----------------- P-tuning-v2 -----------------
        if memory_states is not None:
            memory_length = memory_states[0].size(1)
        elif past_key_values is not None:
            memory_length = past_key_values[0].size(3)
        elif prompt is not None:
            memory_length = prompt.size(1)
        else:
            memory_length = 0
        # ----------------- P-tuning-v2 -----------------

        # attention mask is the beginning postion of B region, \in [0, query_len)
        is_scalar = attention_mask.numel() == 1
        is_sep = is_scalar or attention_mask.numel() == batch_size
        if is_sep:
        ...
        if prompt is not None:
            hidden_states = torch.cat((prompt, hidden_states), 1)
        # ----------------- P-tuning -----------------
        ...
        for i, layer in enumerate(self.layers):
            ...
            past_key_values_i = past_key_values[i] if past_key_values is not None else None
            ...
            if self.checkpoint_activations:
                ...
                hidden_states = layer(*args, mem=mem_i, past_key_values=past_key_values_i)
    ...
    def forward(self, hidden_states, position_ids, attention_mask, memory_states=None, past_key_values=None, prompt=None):
...
class GLMModel(GLMPreTrainedModel):
    ...
    def execute(
        ...
        # -------------- P-tuning-v2 ----------------
        if past_key_values is not None: # past_key_values: [n_layer, 2, bs, n_head, pre_seq_len, n_embed]
            position_ids[:,0] = position_ids[:,0] + past_key_values[0].size(3)
        # -------------- P-tuning-v2 ----------------

        # ----------------- P-tuning -----------------
        if prompt is not None:
            position_ids[:,0] = position_ids[:,0] + prompt.size(1)
        # ----------------- P-tuning -----------------

        # ----------------- P-tuning-v2 mems style -----------------
        # if mems is not None:
        #     position_ids[:, 0] = position_ids[:, 0] + mems[0].size(1)
        # ----------------- P-tuning-v2 mems style ----------------- 

        # ----------------- PTP-RG -------------------
        # 为 embeddings 添加一个服从 (0, 0.00001) 的 Gaussian noise
        # num = 5
        # mean = 0.0
        # std = 0.00001
        ...
        # ptp_rg = torch.zeros(embeddings.shape)
        # for i in range(batch_size):
        #     random_indices = torch.randperm(seq_len)[:num]
        #     random_values = torch.normal(mean, std, size=(num, hidden_size))
        #     ptp_rg[i, random_indices] = random_values
        
        # ptp_rg = ptp_rg.to(embeddings.device)
        # embeddings = embeddings + ptp_rg
        # ----------------- PTP-RG -------------------
        ...
        # Transformer.
        transformer_output = self.transformer(embeddings, position_ids, attention_mask, mems, past_key_values=past_key_values, prompt=prompt)
